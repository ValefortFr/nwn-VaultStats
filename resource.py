...
class ContentObject(object):
    ...
    def from_file(filename):
        ...
        return ContentObject(basename, Extensions[ext.lower()], abspath, 0, size, abspath)
    ...
    def __init__(self, path, only_nwn=True):
        ...
        for dirname, dirnames, filenames in os.walk(self.path):
            for filename in filenames:
                if not only_nwn or os.path.splitext(filename)[1][1:].lower() in Extensions:
...
class BICDirectoryContainer(Container):
    """A Container that directly wraps a directory (e.g. override/).
    ...
    def __init__(self, path):
        super(BICDirectoryContainer, self).__init__()
        if not os.path.isdir(path):
            msg = "Path: %s is not a directory!" % path
            raise ValueError(msg)
        self.path = path
        for dirname, dirnames, filenames in os.walk(self.path):
            for filename in filenames:
                if os.path.splitext(filename)[1][1:].lower() in ['bic']:
                    self.add_file(os.path.join(dirname, filename))
